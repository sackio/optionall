...
(function(){
  ...
  var Options = function(){
    ...
    var cascade_value = function(array){
      return _.reduce(array, function(e, s){ return e || s; }, undefined);
    };

    //get path relative to config files
    O.__dirname = settings.__dirname || cascade_value(settings.dirname_priority);

    //get name of environment
    O.environment = O.env = settings.environment || settings.env || cascade_value(settings.environment_priority);
    ...
    //when a script is called with a method argument, apply the object's method
    O.commandline = function(mod, o){
      if (require.main !== mod || !o.method) return;
      var method = Belt.deepProp(mod, 'exports.' + o.method);
      if (!method) return;
      return method.apply(mod, Belt.toArray(o.args || []).concat([o]));
    };
